...
\newtcolorbox{quotebox}{enhanced, colframe=MyOrange, top=0.2cm, bottom=0.2cm, colback=orange!15}
...
%	########### Theorie #############
	...
	\begin{frame}{Analyse}
		...
		\begin{quotebox}\blockcquote{Hubble1929}{Two solutions have been made, ...}\end{quotebox}
	...
	\begin{frame}{Intrepretation von Hubble}
		...
		\begin{quotebox}
			\begin{itemize}
				...
				Mit Faktor \( K = 465(50) \unit{km/s/Mpc} \), \( K = 513(60) \unit{km/s/Mpc} \)}
...
%	######### folgeexperimente #########
	...
	\begin{frame}{Messung über SN1a}
		\centering
		\begin{tikzpicture}
			\node[anchor=south west,inner sep=0] (image) at (0,0) {\includegraphics[height=0.9\textheight]{Sn1a}};
			
			\begin{scope}[x={(image.south east)},y={(image.north west)}]
%				\draw[line width=1pt] (0.09, 0.355) -- (1, 0.93);
				\draw[line width=1pt] (0.09, 0.355) -- (1, 0.89);
...
%				\draw[fill] (0.5, 0.59) circle (.005) node [below] {\( \approx 1.4 \unit{\giga\year} \)};
	...
	\begin{frame}{Krise}
		\begin{tikzpicture}
			\node[anchor=south west,inner sep=0] (image) at (0,0) {\includegraphics[height=0.9\textheight]{crisis2}};
