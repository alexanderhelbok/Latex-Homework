...
%	##################### Intro #######################
	\begin{frame}{Overview}
		\begin{tikzpicture}[scale=2.25, every node/.style={scale=0.8, black}]
			...
			\draw[MyBlue, line width=1pt] (0.8, 0) -- (0.8, 0.35) node [left, xshift=0cm] {Shor (1994)};
%			
			\draw[MyBlue, line width=1pt] (1, 0) -- (1, 0.5) node [above left, xshift=0.5cm] {Trapped ion CNOT (1995)};
%			
			\draw[MyOrange, line width=1pt] (1.8, 0) -- (1.8, 0.85) node [above left, xshift=0.5cm] {First SC qubit (1999)};
%			
			\draw[MyBlue, line width=1pt] (2.2, 0) -- (2.2, 1.2) node [above left, xshift=0.5cm] {Rabi Oscillations (2001)};
%			 
			\draw[MyBlue, line width=1pt] (2.8, 0) -- (2.8, 1.55) node [above left, xshift=0.5cm] {cQED (2004)};
%			
			\draw[MyBlue, line width=1pt] (3.4, 0) -- (3.4, 1.9) node [above left, xshift=0.5cm] {Transmon (2007)};
%			
			\draw[MyPurple, line width=1pt] (3.8, 0) -- (3.8, 2.4) node [above left, xshift=0.5cm] {2 qubit entanglement(2009)};
		\end{tikzpicture}
	\end{frame}
	...
	\begin{frame}{Superconducting circuits}
		...
		\begin{textblock*}{5cm}(.85\textwidth,.2\textheight)
			\begin{circuitikz}[scale=1, transform shape]				
				...
				to (2.4, 0.5)
				to[color=MyOrange, josephson, /tikz/circuitikz/bipoles/length=0.8cm, line width=0.8pt] (2.4, 1.5)
				to (1.6, 1.5)
				to[color=MyOrange, josephson, /tikz/circuitikz/bipoles/length=0.8cm, line width=0.8pt] (1.6, 0.5)
				to (2, 0.5);
...
%				\node[below right] at (2.3, 1) {\( L_J \)};
		...
		\begin{textblock*}{5cm}(.46\textwidth,.6\textheight)
			\begin{tikzpicture}
				\begin{axis}[MyStyle1,
					scale=0.775,
					xlabel = phase \(\phi\)]
		...
		\begin{textblock*}{5cm}(.82\textwidth,.6\textheight)
			\begin{tikzpicture}
				\begin{axis}[
					scale=0.775,
					width=\textwidth, height=4.5cm,
					...
					\addplot[line width=0.8pt, MyRed, smooth] table [x=x, y=w02, col sep=comma] {bilder/asym_transmon_e.csv};
				\end{axis}
			\end{tikzpicture}
		\end{textblock*}
		...
		\begin{columns}
			\begin{column}{.5\textwidth}
				...
				\begin{tikzpicture}[every node/.style={font=\scriptsize, align=left}]
					\node[anchor=south west,inner sep=0] (image) at (0,0) {\includegraphics[height=0.7\textheight, trim={9.85cm 0 0 0}, clip]{Figure1_DeviceandSpec_LoRes}};
					
					\begin{scope}[x={(image.south east)},y={(image.north west)}]
						\node[MyBlue] at (0.3, 0.8) {\( T_1 = 1.3 \unit{\micro s} \)};
						\node[MyBlue] at (0.3, 0.73) {\( T_2^* = 1.8 \unit{\micro s} \)};
						\node[MyRed] at (0.3, 0.42) {\( T_1 = 0.8 \unit{\micro s} \)};
						\node[MyRed] at (0.3, 0.35) {\( T_2^* = 1.2 \unit{\micro s} \)};
					\end{scope}
				\end{tikzpicture}
			\end{column}
			\begin{column}{.5\textwidth}
...
%					\end{scope}
			...
			\begin{column}{.5\textwidth}
				\begin{align*}
					...
					\bm{\phi}_{\MyRed{1}\MyBlue{1}} &= \bm{\phi}_{\MyRed{0}\MyBlue{1}} + \bm{\phi}_{\MyRed{1}\MyBlue{0}} - \uint{\zeta(t)}{t}
				\end{align*}
			\end{column}
		\end{columns}
	\end{frame}
	...
	\begin{frame}{Qubit entangling}
		\centering
		\begin{tikzpicture}[every node/.style={font=\scriptsize, align=left}]
			\node[anchor=south west,inner sep=0] (image) at (1,0) {\includegraphics[height=.8\textheight, trim={0 0 0 0}, clip]{Bell}};
	...
	\begin{frame}{Outlook}
		\begin{tikzpicture}[scale=2.25, every node/.style={scale=0.8, black, line width=1pt}]
			\draw[line width=1.5pt] (-0.15, 0) -- (4.15, 0);
