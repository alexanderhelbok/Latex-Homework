	...
	\begin{enumerate}
		...
		\begin{tikzpicture}
			\begin{axis}[
				...
				tick align=outside,
				xmin=0,xmax=2.4*pi,ymin=-1.4,ymax=1.2,
				xlabel style={below},
				ylabel style={left},
				...
				%legend columns=2,
				legend style={at={(axis description cs:0.4,0.9)},anchor=east}]
				\addplot[domain=0:2.4*pi, samples=200, blue, thick] {-sin(deg(x))};
				\addplot[domain=0:2.4*pi, samples=200, orange, thick] {cos(deg(x))};
			 	\draw[dashed,olive,<->] (axis cs: 0.5*pi,-1.05) -- node[below, text=black,font=\footnotesize]{$\phi =  \tfrac{\pi}{2}$}(axis cs: pi,-1.05);
				\legend{$q(t)$~~~,$I(t)$};
			\end{axis}
		\end{tikzpicture}\\
	\tcbline
...
\begin{mybox}{2. Serienschwingkreis}
	...
	\begin{enumerate}
		...
		\begin{flalign*}
			...
			I_0 &= \dl{\tfrac{U_0}{R + L\omega\iu - \tfrac{\iu}{\omega C}}} &&
		\end{flalign*}
	\tcbline
		\item \(  \)
		\begin{flalign*}
...
\begin{mybox}{3. Elektrische Verschiebung}
	\centering \( \epsilon_1 = 2\epsilon_0;\quad \epsilon_2 = 1.5\epsilon_0\)
	\tcblower
	\begin{enumerate}
		\item \(  \)
		\begin{flalign*}
	...
	\tcbline
		\item \(  \)
		\begin{flalign*}
			\vec{E}_1 &= \tfrac{\vec{D}_1}{\epsilon_1} = \dl{\tfrac{\sigma}{2\epsilon_0} \hat{z}} &&\\
			\vec{E}_2 &= \tfrac{\vec{D}_2}{\epsilon_2} = \dl{\tfrac{3\sigma}{2\epsilon_0} \hat{z}} &&
		\end{flalign*}
	\tcbline
		\item \(  \)
		\begin{flalign*}
			\vec{P}_1 &= \vec{D}_1 - \epsilon_0\vec{E}_1 = \dl{\tfrac{\sigma}{2} \hat{z}} &&\\
			\vec{P}_2 &= \vec{D}_2 - \epsilon_0\vec{E}_2 = \dl{\tfrac{\sigma}{3} \hat{z}} &&
		\end{flalign*}
	\tcbline
		\item \(  \)
		\begin{flalign*}
			C &= \tfrac{1}{\tfrac{a}{A\epsilon_1} + \tfrac{a}{A\epsilon_2}} = \tfrac{A\epsilon_1\epsilon_2}{a(\epsilon_1 + \epsilon_2)} = \tfrac{6A\epsilon_0}{7a} &&\\
			U &= \tfrac{Q}{C} = \sigma \tfrac{a(\epsilon_1 + \epsilon_2)}{\epsilon_1\epsilon_2} = \dl{\sigma \tfrac{7a}{6\epsilon_0}} &&
		\end{flalign*}
	\tcbline
		\item From Top to Bottom: \( \sigma, -\tfrac{\sigma}{2}, \tfrac{\sigma}{2}, -\tfrac{\sigma}{3}, \tfrac{\sigma}{3}, -\sigma \)
	\tcbline
		\item \(  \)
		\begin{flalign*}
			\vec{E}_1 &= \tfrac{\tfrac{\sigma}{2}}{\epsilon_0} = \dl{\tfrac{\sigma}{2\epsilon_0}} &&\\
			\vec{E}_2 &= \tfrac{\tfrac{2\sigma}{3}}{\epsilon_0} = \dl{\tfrac{2\sigma}{3\epsilon_0}} &&	
		\end{flalign*}
	\end{enumerate}
\end{mybox}
