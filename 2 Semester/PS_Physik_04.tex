...
\ExplSyntaxOn
\NewDocumentCommand{\cuts}{>{\SplitArgument{1}{;}}D[]{}}{%
	\int\limits_{\use_i:nn#1}^{\use_ii:nn#1}%
}
\ExplSyntaxOff
...
\begin{mybox}{1. Der Dipol}
	...
	\tcbline
		...
		\begin{flalign*}
			E_{pot} &= \int\limits_{\theta}^{\pi/2}\ \abs*{\vec{M}}\ \mathrm{d}\tilde{\theta} = pE \int\limits_{\theta}^{\pi/2}\ \sin(\tilde{\theta})\ \mathrm{d}\tilde{\theta} = pE\cos(\theta) &&\\
			&= \dl{\vec{p} \cdot \vec{E}} &&
		\end{flalign*}
	\end{enumerate}
\end{mybox}
