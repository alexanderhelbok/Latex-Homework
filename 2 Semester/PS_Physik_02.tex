...
%			<--- z ---->
		...
		\end{center}	
		\begin{flalign*}
			\mathrm{d}E_1 &= k \tfrac{\mathrm{d}Q}{h^2} \cos(\theta) = k \tfrac{\lambda \mathrm{d}r}{r^2 + z^2} \tfrac{z}{\sqrt{r^2 + z^2}} = k \tfrac{\lambda rz}{(r^2 + z^2)^{3/2}} \mathrm{d}\varphi &&\\
			E_1 &= \int\ \mathrm{d}E_1 = k \tfrac{\lambda rz}{(r^2 + z^2)^{3/2}} \int\limits_{0}^{2\pi}\ \mathrm{d}\varphi = k \tfrac{\lambda2\pi rz}{(r^2 + z^2)^{3/2}}
